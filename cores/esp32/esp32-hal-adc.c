...
uint16_t IRAM_ATTR __CanalogRead(uint8_t pin)
{
...
void __CanalogReadResolution(uint8_t bits)
{
    if(!bits || bits > 16){
        return;
    }
...
int __hallRead()    //hall sensor without LNA
{
    ...
    Sens_Vp0 = __CanalogRead(36);
    Sens_Vn0 = __CanalogRead(39);
    SET_PERI_REG_MASK(RTC_IO_HALL_SENS_REG, RTC_IO_HALL_PHASE);
    Sens_Vp1 = __CanalogRead(36);
    Sens_Vn1 = __CanalogRead(39);
...
extern uint16_t CanalogRead(uint8_t pin) __attribute__ ((weak, alias("__CanalogRead")));
//extern uint16_t analogRead(uint8_t pin) __attribute__ ((weak, alias("__analogRead")));


extern void CanalogReadResolution(uint8_t bits) __attribute__ ((weak, alias("__CanalogReadResolution")));
