...
static bool i2cCheckLineState(int8_t sda, int8_t scl){
    ...
    CdigitalWrite(sda, HIGH);
    CdigitalWrite(scl, HIGH);
    CpinMode(sda, PULLUP|OPEN_DRAIN|INPUT);
    CpinMode(scl, PULLUP|OPEN_DRAIN|OUTPUT);

    if(!CdigitalRead(sda) || !CdigitalRead(scl)) { // bus in busy state
        log_w("invalid state sda(%d)=%d, scl(%d)=%d", sda, CdigitalRead(sda), scl, CdigitalRead(scl));
        CdigitalWrite(scl, HIGH);
        for(uint8_t a=0; a<9; a++) {
            delayMicroseconds(5);
            CdigitalWrite(scl, LOW);
            delayMicroseconds(5);
            CdigitalWrite(scl, HIGH);
            if(CdigitalRead(sda)){ // bus recovered
    ...
    if(!CdigitalRead(sda) || !CdigitalRead(scl)) { // bus in busy state
        log_e("Bus Invalid State, TwoWire() Can't init sda=%d, scl=%d",CdigitalRead(sda),CdigitalRead(scl));
...
i2c_err_t i2cAttachSCL(i2c_t * i2c, int8_t scl)
{
    ...
    CdigitalWrite(scl, HIGH);
    CpinMode(scl, OPEN_DRAIN | PULLUP | INPUT | OUTPUT);
...
i2c_err_t i2cAttachSDA(i2c_t * i2c, int8_t sda)
{
    ...
    CdigitalWrite(sda, HIGH);
    CpinMode(sda, OPEN_DRAIN | PULLUP | INPUT | OUTPUT );
