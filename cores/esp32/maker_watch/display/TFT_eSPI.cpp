...
#ifdef TFT_CS
  CdigitalWrite(TFT_CS, HIGH); // Chip select high (inactive)
  CpinMode(TFT_CS, OUTPUT);
#endif

// Configure chip select for touchscreen controller if present
#ifdef TOUCH_CS
  CdigitalWrite(TOUCH_CS, HIGH); // Chip select high (inactive)
  CpinMode(TOUCH_CS, OUTPUT);
#endif

#ifdef TFT_WR
  CdigitalWrite(TFT_WR, HIGH); // Set write strobe high (inactive)
  CpinMode(TFT_WR, OUTPUT);
#endif

#ifdef TFT_DC
  CdigitalWrite(TFT_DC, HIGH); // Data/Command high = data mode
  CpinMode(TFT_DC, OUTPUT);
#endif

#ifdef TFT_RST
  if (TFT_RST >= 0) {
    CdigitalWrite(TFT_RST, HIGH); // Set high, do not share pin with another SPI device
    CpinMode(TFT_RST, OUTPUT);
  }
#endif
  ...
  CdigitalWrite(TFT_RD, HIGH);
  CpinMode(TFT_RD, OUTPUT);
...
void TFT_eSPI::init(uint8_t tc)
{
...
#if defined(ESP32_PARALLEL)
    CdigitalWrite(TFT_CS, LOW); // Chip select low permanently
    CpinMode(TFT_CS, OUTPUT);
#else
  #ifdef TFT_CS
...
#ifdef TFT_RST
  if (TFT_RST >= 0) {
    CdigitalWrite(TFT_RST, HIGH);
    delay(5);
    CdigitalWrite(TFT_RST, LOW);
    delay(20);
    CdigitalWrite(TFT_RST, HIGH);
  }
  ...
  CdigitalWrite(TFT_BL, TFT_BACKLIGHT_ON);
  CpinMode(TFT_BL, OUTPUT);
#else
  #if defined (TFT_BL) && defined (M5STACK)
    // Turn on the back-light LED
    CdigitalWrite(TFT_BL, HIGH);
    CpinMode(TFT_BL, OUTPUT);
  #endif
#endif
}
...
{//*
  ...
  CpinMode(TFT_D0, mode);
  CpinMode(TFT_D1, mode);
  CpinMode(TFT_D2, mode);
  CpinMode(TFT_D3, mode);
  CpinMode(TFT_D4, mode);
  CpinMode(TFT_D5, mode);
  CpinMode(TFT_D6, mode);
  CpinMode(TFT_D7, mode);
  return; //*/
}
...
uint8_t TFT_eSPI::tft_Read_8(void)
{
  ...
  for (uint8_t i = 0; i < 8; i++) {  // read results
    ret <<= 1;
    SCLK_L;
    if (CdigitalRead(TFT_MOSI)) ret |= 1;
    SCLK_H;
  }
...
//#include "Extensions/Sprite.cpp"  // derectly include for maker watch
